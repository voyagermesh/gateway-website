...
.PHONY: docs
docs: hugo-tools
	$(HUGO_TOOLS) docs-aggregator --fm-replacements product_gateway_=docs_,/products/gateway/=/docs/
...
docs-skip-assets: hugo-tools
	$(HUGO_TOOLS) docs-aggregator --exclude-assets --fm-replacements product_gateway_=docs_,/products/gateway/=/docs/
	find ./data -name "*.json" -exec sed -i 's/https:\/\/cdn.appscode.com\/images/\/assets\/images/g' {} \;

.PHONY: assets
assets: hugo-tools
	$(HUGO_TOOLS) docs-aggregator --only-assets --fm-replacements product_gateway_=docs_,/products/gateway/=/docs/
	find ./data -name "*.json" -exec sed -i 's/https:\/\/cdn.appscode.com\/images/\/assets\/images/g' {} \;
	rm -rf static/files/cluster-api
	rm -rf static/files/cluster-api-provider-aws
	rm -rf static/files/cluster-api-provider-azure
